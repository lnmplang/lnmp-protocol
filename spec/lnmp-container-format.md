...
Reference fixtures for the valid headers above now live under `spec/examples/container/` and are automatically checked by `lnmp-verify-examples` so that documentation and parser behavior never drifts. `text_mode.hex` and `binary_mode.hex` now embed real LNMP payloads that are compared against the canonical fixtures in `spec/examples/text/` and `spec/examples/binary/`. The same directory contains `invalid_*.hex` fixtures that exercise each error in the mapping below; the verifier expects them to fail with the documented reason, keeping the spec and parser diagnostics in lockstep.
