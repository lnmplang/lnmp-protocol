...
> **FID Registry:** All FID examples in this guide use official Field IDs from [`registry/fids.yaml`](../../registry/fids.yaml).
...
inner.add_field(LnmpField { fid: 12, value: LnmpValue::Int(1) });  // F12=user_id
...
// Use text format for nested structures (F70=nested_data)
let text = "F70={F12=1;F7=1}";
let mut parser = Parser::new(text).unwrap();
let record = parser.parse_record().unwrap();
...
// Record array: F71=[{F20=alice},{F20=bob}] (F71=record_list)
...
```rust
// v0.1 output (insertion order)
F30=3
F7=1
F12=2

// v0.2 output (sorted by FID)
F7=1
F12=2
F30=3
```
