...
fn test_roundtrip_integers_positive() {
    let test_values = vec![0, 1, 42, 127, 128, 255, 256, 14532, 65535, 1000000, i64::MAX];
...
fn test_roundtrip_integers_negative() {
    let test_values = vec![-1, -42, -127, -128, -255, -256, -14532, -65535, -1000000, i64::MIN];
...
fn test_roundtrip_all_types_combined() {
    ...
    assert_eq!(decoded_record.get_field(2).unwrap().value, LnmpValue::Int(-123));
    assert_eq!(decoded_record.get_field(3).unwrap().value, LnmpValue::Float(3.14159));
    assert_eq!(decoded_record.get_field(4).unwrap().value, LnmpValue::Float(-2.718));
    assert_eq!(decoded_record.get_field(5).unwrap().value, LnmpValue::Bool(true));
    assert_eq!(decoded_record.get_field(6).unwrap().value, LnmpValue::Bool(false));
    assert_eq!(decoded_record.get_field(7).unwrap().value, LnmpValue::String("hello world".to_string()));
    assert_eq!(decoded_record.get_field(8).unwrap().value, LnmpValue::String("emoji: 🎯".to_string()));
    assert_eq!(
        decoded_record.get_field(9).unwrap().value,
        LnmpValue::StringArray(vec!["a".to_string(), "b".to_string(), "c".to_string()])
    );
    assert_eq!(decoded_record.get_field(10).unwrap().value, LnmpValue::StringArray(vec![]));
}
...
fn test_unsorted_fields_become_sorted() {
    ...
    assert_eq!(decoded_record.get_field(5).unwrap().value, LnmpValue::Int(1));
    assert_eq!(decoded_record.get_field(50).unwrap().value, LnmpValue::Int(2));
    assert_eq!(decoded_record.get_field(100).unwrap().value, LnmpValue::Int(3));
}
...
fn test_canonical_stability_ten_iterations() {
    ...
    for i in 0..10 {
        ...
        assert_eq!(
            current_binary, canonical_binary,
...
fn test_canonical_form_preserves_data_integrity() {
    ...
    // Verify values
    assert_eq!(decoded_record.get_field(1).unwrap().value, LnmpValue::Int(-12345));
    assert_eq!(decoded_record.get_field(25).unwrap().value, LnmpValue::String("hello\nworld".to_string()));
    assert_eq!(decoded_record.get_field(50).unwrap().value, LnmpValue::Float(2.718281828));
    assert_eq!(decoded_record.get_field(75).unwrap().value, LnmpValue::Bool(false));
    assert_eq!(
