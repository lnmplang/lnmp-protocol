...
fn test_v0_5_type_tag_0x06_not_yet_implemented() {
    ...
    match result {
        Err(BinaryError::InvalidValue { type_tag, reason, .. }) => {
            ...
            assert!(reason.contains("not yet implemented"), "Expected 'not yet implemented' in error message");
        }
...
fn test_malformed_varint_incomplete() {
    ...
    match result {
        Err(BinaryError::InvalidValue { field_id, reason, .. }) => {
...
fn test_unsorted_fields_detected_with_validation() {
    ...
    match result {
        Err(BinaryError::CanonicalViolation { reason }) => {
            assert!(reason.contains("not in ascending FID order") || reason.contains("F5") && reason.contains("F10"));
        }
