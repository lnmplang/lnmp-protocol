...
use lnmp_codec::binary::{
    BinaryDecoder, BinaryEncoder, DecoderConfig, DeltaConfig, DeltaDecoder, DeltaEncoder,
    EncoderConfig, StreamingConfig, StreamingDecoder, StreamingEncoder,
};
    ...
    fn build_nested(current_depth: usize, max_depth: usize, breadth: usize) -> LnmpRecord {
        ...
        for i in 0..breadth {
            ...
            if current_depth < max_depth {
                ...
                record.add_field(lnmp_core::LnmpField {
                    fid,
                    value: LnmpValue::NestedRecord(Box::new(build_nested(
                        current_depth + 1,
                        max_depth,
                        breadth,
                    ))),
...
fn bench_nested_encoding(c: &mut Criterion) {
    ...
    for depth in [1, 2, 3, 5] {
        ...
        group.bench_with_input(BenchmarkId::new("depth", depth), &record, |b, record| {
            b.iter(|| black_box(encoder.encode(black_box(record)).unwrap()));
        });
    }
...
fn bench_nested_decoding(c: &mut Criterion) {
    ...
    for depth in [1, 2, 3, 5] {
        ...
        group.bench_with_input(BenchmarkId::new("depth", depth), &encoded, |b, encoded| {
    ...
    for change_pct in [10, 25, 50, 75] {
        ...
        group.bench_with_input(
            ...
            |b, (base, modified)| {
                b.iter(|| {
                    let delta = encoder
                        .compute_delta(black_box(base), black_box(modified))
                        .unwrap();
...
fn bench_delta_application(c: &mut Criterion) {
    ...
    for change_pct in [10, 25, 50, 75] {
        ...
        group.bench_function(BenchmarkId::new("changes_pct", change_pct), |b| {
            b.iter(|| {
                let config = DeltaConfig::new().with_enable_delta(true);
                let decoder = DeltaDecoder::with_config(config);
                let delta = decoder
                    .decode_delta(black_box(&encoded_delta_owned))
                    .unwrap();
...
fn bench_v04_vs_v05_encoding(c: &mut Criterion) {
    ...
    for num_fields in [10, 50, 100, 500] {
        ...
        group.bench_with_input(
            ...
            &record,
            |b, record| {
                b.iter(|| black_box(v04_encoder.encode(black_box(record)).unwrap()));
            },
        );
        ...
        group.bench_with_input(
            ...
            &record,
            |b, record| {
                b.iter(|| black_box(v05_encoder.encode(black_box(record)).unwrap()));
            },
        );
    }
...
fn bench_v04_vs_v05_decoding(c: &mut Criterion) {
    ...
    for num_fields in [10, 50, 100, 500] {
        ...
        group.bench_with_input(
            ...
            &v04_encoded,
            |b, encoded| {
                b.iter(|| black_box(v04_decoder.decode(black_box(encoded)).unwrap()));
            },
        );
        ...
        group.bench_with_input(
            ...
            &v05_encoded,
            |b, encoded| {
                b.iter(|| black_box(v05_decoder.decode(black_box(encoded)).unwrap()));
            },
        );
    }
