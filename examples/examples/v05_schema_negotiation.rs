...
fn successful_negotiation() {
    ...
    println!(
        "     Streaming: {}",
        client_caps.features.supports_streaming
    );
    ...
    println!(
        "     Streaming: {}",
        server_caps.features.supports_streaming
    );
...
fn feature_flag_negotiation() {
    ...
    {
        if let NegotiationResponse::SendMessage(msg3) =
            client_negotiator.handle_message(msg2).unwrap()
        {
            if let NegotiationResponse::SendMessage(msg4) =
                server_negotiator.handle_message(msg3).unwrap()
            {
        ...
        match client_negotiator.handle_message(msg2) {
            Ok(NegotiationResponse::SendMessage(msg3)) => {
                match server_negotiator.handle_message(msg3).unwrap() {
                    NegotiationResponse::Complete(_) => println!("   ✗ Unexpected success"),
                    NegotiationResponse::Failed(e) => {
                        println!("   ✓ Correctly detected type mismatch: {}", e)
                    }
...
fn version_mismatch_example() {
    ...
    match server_negotiator.handle_message(msg1).unwrap() {
        NegotiationResponse::SendMessage(msg2) => match client_negotiator.handle_message(msg2) {
